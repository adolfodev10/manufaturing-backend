...
const ConfiguracoesRoutes = async (app) => {
    // GET – obter todas as configurações
    app.withTypeProvider().get("/configuracoes", {}, async (req, reply) => {
        try {
            // Buscar todas as linhas da tabela system_config
            const configs = await prismaclient_1.prisma.system_config.findMany();
    ...
    app.withTypeProvider().put("/configuracoes", {
        schema: {
            body: zod_1.z.record(zod_1.z.string(), zod_1.z.any()), // aceita qualquer objecto com chaves string
        },
    }, async (req, reply) => {
        ...
        try {
            ...
            for (const key of Object.keys(updates)) {
                await prismaclient_1.prisma.system_config.upsert({
                    where: { key },
